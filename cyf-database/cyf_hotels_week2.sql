...
-- Solutions
-- Ex 1. 
ALTER TABLE customers RENAME TO clients;  -- re-naming a table

ALTER TABLE customers ADD COLUMN date_of_birth DATE; --adding a column
ALTER TABLE customers RENAME COLUMN date_of_birth TO birthdate ; -- re-naming a column
ALTER TABLE customers DROP COLUMN birthdate; -- deleting a column

-- Ex 2. 
CREATE TABLE test(); -- creating a table
DROP TABLE test; -- deleting a table
...
-- Ex 5. 
SELECT bookings.checkin_date, customers.name, hotels.name FROM bookings INNER JOIN customers ON bookings.customer_id=customers.id INNER JOIN hotels ON hotels.id=bookings.hotel_id;


SELECT * FROM customers INNER JOIN bookings ON bookings.customer_id=customers.id WHERE checkin_date > '2020-01-01';

SELECT customers.name, bookings.checkin_date, bookings.nights 
FROM customers INNER JOIN bookings ON customers.id=bookings.customer_id
               INNER JOIN hotels ON bookings.hotel_id=hotels.id
WHERE hotels.name='Jade Peaks Hotel';


SELECT customers.name, bookings.checkin_date, hotels.name, bookings.nights
FROM bookings INNER JOIN customers ON customers.id=bookings.customer_id
               INNER JOIN hotels ON bookings.hotel_id=hotels.id
WHERE bookings.nights > 5;

-- Ex 6
SELECT * FROM customers WHERE name LIKE 'S%';
SELECT * FROM hotels WHERE upper(name) LIKE '%HOTEL%';
SELECT * FROM hotels WHERE lower(name) LIKE '%hotel%';
SELECT upper(name), lower(name) FROM hotels;

SELECT bookings.checkin_date, customers.name, hotels.name, bookings.nights 
FROM bookings INNER JOIN customers ON bookings.customer_id = customers.id 
